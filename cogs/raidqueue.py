...
class RaidQueue(commands.Cog):
    ...
    async def autoraid_task(self, inter):
        ...
        while self.raidtask_running:
            ...
            try:
                ...
                # raid lobby
                await wait_for(
                    self.bot.sbbcon.raid_lobby(self.raid_queue),
                    timeout=200.0
                    )
                ...
                await self.bot.sbbcon.error_screenshot()
                self.log.error("TimeoutError raid is restarting")
                continue
        ...
        await self.bot.sbbcon.detach_controller()
        self.log.info("Controller detached")
        await inter.send(f"Finished raiding, {raid_counter} raids were done!")
    ...
    async def open_queue(
        ...
        await modal_inter.response.send_message("Queue is open!!!")
